...
const CONFIG = [
  ...
  { key: "maintainability", label: "Maintenabilité", icon: Wrench },
  { key: "scalability", label: "Évolutivité", icon: TrendingUp },
  { key: "quality", label: "Qualité du code", icon: BadgeCheck },
] as const;

export function Optimizations({ optimizations }: { optimizations: OptimizationsType }) {
  return (
    <CaseStudySection eyebrow="Optimisations" title="Décisions d'ingénierie">
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {CONFIG.map(({ key, label, icon: Icon }, i) => {
